...
echo "<div class=\"row\">"
echo "<h3 class=\"page-header\">Staff</h3>"
for i in people/staff/*.info; do
...
echo "<div class=\"row\">"
echo "<h3 class=\"page-header\">Collaborators</h3>"
for i in people/collaborators/*.info; do
    [ -f $i ] || fail "$i is not a valid file"
    gen_card "$i"
done
echo "</div>"
...
for i in people/students-phd/*.info; do
    [ -f $i ] || fail "$i is not a valid file"
    gen_card "$i"
done
echo "</div>"
...
for i in people/students-msc/*.info; do
    [ -f $i ] || fail "$i is not a valid file"
    gen_card "$i"
done
echo "</div>"
...
for i in people/students-undergrad/*.info; do
    [ -f $i ] || fail "$i is not a valid file"
    gen_card "$i"
done
echo "</div>"
